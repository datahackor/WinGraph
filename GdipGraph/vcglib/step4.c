...
static void gs_ide1515(int le, int mr, int miy, int may);
...
void step4_main(void)
{
	...
	if ((G_orientation == 3) || (G_orientation == 2)) gs_ide1524();
	if ((G_orientation == 1) || (G_orientation == 2)) gs_ide1521();
	calc_max_xy_pos();
...
void calc_node_ports(GNODE v, int xypos_avail)
{
	...
	;
	act_port = 1;
	if (G_portsharing != 1) act_port = 0;
	if (v->anchordummy) act_port = 0;
	a = v->succ;
	if (a) {
		act_pstyle = a->kante->arrowstyle2;
		act_pcol = a->kante->arrowcolor2;
		act_psize = a->kante->arrowsize2;
		if (a->kante->kantenart == 'r') act_pstyle = -1;
		else if (a->kante->kantenart == 'l') act_pstyle = -1;
	} while (a) {
		pstyle = a->kante->arrowstyle2;
		pcol = a->kante->arrowcolor2;
		psize = a->kante->arrowsize2;
		if (a->kante->kantenart == 'r') pstyle = -1;
		else if (a->kante->kantenart == 'l') pstyle = -1;
		if (v->anchordummy) pstyle = 5 + (act_pstyle + 1) % 20;
		if (G_portsharing != 1) pstyle = 5 + (act_pstyle + 1) % 20;
		if ((psize == act_psize) && (pcol == act_pcol) && (pstyle == act_pstyle)) {
			if (pstyle != -1) a->kante->weights = act_port;
		}
		else {
			act_pstyle = pstyle;
			act_psize = psize;
	...
	if (xypos_avail) {
		switch (v->shape) {
		case 2: case 1: midport = 1;
			nullport = 0;
			a = v->succ;
			while (a) {
				if (a->kante->end->xloc + a->kante->end->width < v->xloc)  portpos = -1;
				else if (a->kante->end->xloc > v->xloc + v->width)  portpos = 1;
				else portpos = 0;
				if (portpos < 0) midport = a->kante->weights;
				if (portpos == 0) {
					if (!nullport)   midport = nullport = a->kante->weights;
					else midport = (a->kante->weights + nullport) / 2;
				}
				else nullport = 0;
				a = a->next;
			} if (act_port - midport > midport - 1)  portpos = 2 * act_port - 3 * midport + 1;
	...
	if (a) {
		...
		if (G_portsharing != 1) pstyle = 5 + (act_pstyle + 1) % 20;
		if ((psize == act_psize) && (pcol == act_pcol) && (pstyle == act_pstyle)) {
			if (pstyle != -1) a->kante->weightp = act_port;
		}
		else {
			act_pstyle = pstyle;
			act_psize = psize;
	...
	if (xypos_avail) {
		switch (v->shape) {
		case 2: case 3: case 1: midport = 1;
			nullport = 0;
			a = v->pred;
			while (a) {
				if (a->kante->start->xloc + ((a->kante->start)->width) < v->xloc)  portpos = -1;
				else if (((a->kante->start)->xloc) > v->xloc + v->width)  portpos = 1;
				else portpos = 0;
				if (portpos < 0) midport = a->kante->weightp;
				if (portpos == 0) {
					if (!nullport)   midport = nullport = a->kante->weightp;
					else midport = (a->kante->weightp + nullport) / 2;
				}
				else nullport = 0;
				a = a->next;
			} if (act_port - midport > midport - 1)  portpos = 2 * act_port - 3 * midport + 1;
...
void calc_edge_xy(GNODE v)
{
	...
	if (c) {
		if (c->target) {
			...
			if (e->start == v) {
				...
				if (e->end->xloc < node_x)  e->btxloc = e->sxloc = node_x + dist1;
				else e->btxloc = e->sxloc = node_x - dist1 + node_width;
			}
			else {
				e->bbyloc = e->eyloc = node_y + node_height / 2;
				if (e->start->xloc < node_x)  e->bbxloc = e->exloc = node_x + dist1;
				else e->bbxloc = e->exloc = node_x - dist1 + node_width;
			}
		} if (c->target2) {
			e = c->edge2;
			if (e->start == v) {
	...
	while (a) {
		...
		case 'l': e->btyloc = e->syloc = node_y + node_height / 2;
			...
			break;
		case 'r': e->btyloc = e->syloc = node_y + node_height / 2;
			e->btxloc = e->sxloc = node_x + node_width - dist1;
			break;
		default:  e->btxloc = e->sxloc = node_x + node_width * e->weights / (node_succports + 1);
			switch (v->shape) {
			case 1: if (e->sxloc - node_x < node_width / 2) dist = ((node_width + 1) / 2 - e->sxloc + node_x) * node_height / node_width;
				  else dist = (-(node_width + 1) / 2 + e->sxloc - node_x) * node_height / node_width;
				e->syloc = node_y + node_height - dist;
				break;
			case 2: dist = node_height / 2 - gstoint(sqrt((double)(node_height * node_height) / 4.0 - (double)(node_height * node_height) / (double)(node_width * node_width) * (e->sxloc - node_x - (double)node_width / 2.0) * (e->sxloc - node_x - (double)node_width / 2.0)));
				e->syloc = node_y + node_height - dist;
				break;
			default:  e->syloc = node_y + node_height;
			} e->btyloc = e->syloc;
		} a = a->next;
	} a = v->pred;
	while (a) {
		e = a->kante;
		switch (e->kantenart) {
		case 'l': e->bbyloc = e->eyloc = node_y + node_height / 2;
			e->bbxloc = e->exloc = node_x + node_width - dist1;
			break;
		case 'r': e->bbyloc = e->eyloc = node_y + node_height / 2;
			e->bbxloc = e->exloc = node_x + dist1;
			break;
		default:  e->bbxloc = e->exloc = node_x + node_width * e->weightp / (node_predports + 1);
			switch (v->shape) {
			case 1: if (e->exloc - node_x < node_width / 2) dist = ((node_width + 1) / 2 - e->exloc + node_x) * node_height / node_width;
				  else dist = (-(node_width + 1) / 2 + e->exloc - node_x) * node_height / node_width;
				e->eyloc = node_y + dist;
				break;
			case 2: dist = node_height / 2 - gstoint(sqrt((double)(node_height * node_height) / 4.0 - (double)(node_height * node_height) / (double)(node_width * node_width) * (e->exloc - node_x - (double)node_width / 2.0) * (e->exloc - node_x - (double)node_width / 2.0)));
				e->eyloc = node_y + dist;
				break;
			case 3: if (e->exloc - node_x < node_width / 2) dist = ((node_width + 1) / 2 - e->exloc + node_x) * node_height * 2 / node_width;
				  else dist = (-(node_width + 1) / 2 + e->exloc - node_x) * node_height * 2 / node_width;
				e->eyloc = node_y + dist;
				break;
			default:  e->eyloc = node_y;
			} e->bbyloc = e->eyloc;
		} a = a->next;
	}
}
static void gs_ide1508(void)
{
	...
	for (i = 0; i <= maxdepth; i++) {
		...
		while (li) {
			...
			if (li->node->yloc + li->node->height > topbendp) topbendp = li->node->yloc + li->node->height;
			li = li->next;
		} li = ((layer[i + 1]).predlist);
		while (li) {
			tpred_connection2[li->node->position] = li;
			if (li->node->yloc < botbendp) botbendp = li->node->yloc;
			li = li->next;
		} maxrow = gs_ide1516(i);
...
static void gs_ide1518(GNODE v, int xpos)
{
	...
	if (k > gs_ide1530) gs_ide1530 = k;
	if (xpos <= gs_ide1540) return;
	gs_ide1540 = xpos;
	a = v->succ;
	while (a) {
		ee = a->kante;
	...
	while (n) {
		m = n->succ;
		if (n->dlx <= xpos) gs_ide1514(n);
		n = m;
	} gs_ide1531 = 0;
	n = gs_ide1538;
	while (n) {
		if (n->dlinfo > gs_ide1531) gs_ide1531 = n->dlinfo;
		n = n->succ;
	}
}
static void gs_ide1517(GNODE v, int xpos)
{
	...
	while (n) {
		m = n->succ;
		if (n->dlx <= xpos) gs_ide1513(n);
		n = m;
	} gs_ide1529 = 0;
	n = gs_ide1526;
	while (n) {
		if (n->dlinfo > gs_ide1529) gs_ide1529 = n->dlinfo;
		n = n->succ;
	}
}
static void gs_ide1501(GEDGE e, GNODE n)
{
	...
	if (!gs_ide1538) gs_ide1538 = d;
	if (gs_ide1539) ((gs_ide1539)->succ) = d;
	gs_ide1539 = d;
	gs_ide1536++;
	if (e->sxloc != e->exloc) gs_ide1531++;
	d->dlinfo = gs_ide1531;
	e->btyloc = gs_ide1531;
	d->dlx = e->sxloc;
}
static void gs_ide1500(GEDGE e, GNODE n)
{
	...
	if (!gs_ide1526) gs_ide1526 = d;
	if (gs_ide1527) ((gs_ide1527)->succ) = d;
	gs_ide1527 = d;
	gs_ide1535++;
	if (e->sxloc != e->exloc) gs_ide1529++;
	d->dlinfo = gs_ide1529;
	e->btyloc = gs_ide1529;
	d->dlx = e->exloc;
}
static void gs_ide1514(DLLIST x)
{
	...
	assert(x->node);
	;
	if (x->pred) x->pred->succ = x->succ;
	else gs_ide1538 = x->succ;
	if (x->succ) x->succ->pred = x->pred;
	else gs_ide1539 = x->pred;
	dllist_free(x);
	gs_ide1536--;
}
static void gs_ide1513(DLLIST x)
{
	...
	;
	if (x->pred) x->pred->succ = x->succ;
	else gs_ide1526 = x->succ;
	if (x->succ) x->succ->pred = x->pred;
	else gs_ide1527 = x->pred;
	dllist_free(x);
	gs_ide1535--;
}
static void gs_ide1515(int level, int maxr, int miny, int maxy)
{
	...
	while (li) {
		...
		while (a) {
			...
			if (one_line_manhatten == 1) {
				...
				if (ee->sxloc >= ee->exloc) k = maxy - (maxy - miny) * (maxr + 1 - ee->btyloc) / (maxr + 1);
				else k = miny + (maxy - miny) * (maxr + 1 - ee->btyloc) / (maxr + 1);
			} ee->btyloc = k;
			ee->bbyloc = k;
			a = a->next;
		} li = li->next;
	...
	while (li) {
		node = li->node;
		if ((node->width == 0) && (node->height == 0) && (node->succ) && node->succ->next && (node->succ->next->next == NULL) && (node->pred == NULL)) {
		...
		} if ((node->width == 0) && (node->height == 0) && (node->pred) && node->pred->next && (node->pred->next->next == NULL) && (node->succ == NULL)) {
	...
	for (i = 0; i <= maxdepth; i++) {
		...
		while (changed) {
			...
			if (j > max_edgebendings) {
				gs_wait_message('t');
				break;
			} if (G_timelimit > 0)  if (test_timelimit(100)) {
				gs_wait_message('t');
				break;
			} changed = 0;
			li = ((layer[i]).succlist);
...
static int gs_ide1509(GNLIST li)
{
	...
	e = li->node->succright;
	assert((e));
	if (e == li->node->succleft) return(bendp);
	h = gs_ide1507(e, li);
	if (h > bendp) bendp = h;
	return(bendp);
}
	...
	if (sx < tx) {
		...
		while (li2) {
			...
			if ((e2) && (e2->btyloc != e2->syloc)) {
				cross = 0;
				if ((e2->start->position < e->start->position) && (e2->end->position > e->end->position)) cross = 1;
				if ((e2->start->position > e->start->position) && (e2->end->position < e->end->position)) cross = 1;
...
static int gs_ide1505(GNLIST li)
{
	...
	e = li->node->predright;
	assert((e));
	if (e == li->node->predleft) return(bendp);
	h = gs_ide1506(e, li);
	if (h < bendp) bendp = h;
	return(bendp);
}
...
static void gs_ide1537(void)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		while (li) {
			node = li->node;
			if ((node->width == 0) && (node->succ) && (node->succ->next == NULL) && (node->pred) && (node->pred->next == NULL)) {
				e1 = node->pred->kante;
				e2 = node->succ->kante;
				my = e2->syloc;
				by = e2->btyloc;
				...
				if ((b2y == e1->eyloc) && (by > my) && (by > ay)) {
					...
					mx = e2->sxloc;
					okay = 1;
					if (((ax < mx) && (mx < e2->bbxloc)) || ((ax < mx) && (G_spline))) {
						li2 = tpred_connection1[node->position];
						li2 = li2->next;
						while (li2) {
							if (okay == 0) break;
							if (!li2->node->anchordummy) {
								kx = li2->node->xloc + li2->node->width;
								if (li2->node->width == 0) kx += G_dspace;
								if (kx <= ax) break;
								ky = li2->node->yloc - 2;
								if (kx <= mx) {
							...
							e2->syloc = e1->bbyloc = e1->eyloc = e2->btyloc;
						}
					}
					else if (((ax > mx) && (mx > e2->bbxloc)) || ((ax > mx) && (G_spline))) {
						li2 = li->next;
						while (li2) {
							if (okay == 0) break;
							if (!li2->node->anchordummy) {
				...
				if ((by == e2->syloc) && (b2y < my) && (b2y < ay)) {
					...
					by = b2y;
					okay = 1;
					if (((ax < mx) && (mx < e1->btxloc)) || ((ax < mx) && (G_spline))) {
						li2 = tpred_connection1[node->position];
						li2 = li2->next;
						while (li2) {
							if (okay == 0) break;
							if (!li2->node->anchordummy) {
								...
								if (li2->node->width == 0) kx += G_dspace;
								if (kx <= ax) break;
								ky = li2->node->yloc + li2->node->height + 2;
								if (kx <= mx) {
									h = ((mx - kx) * (ay - by)) / (mx - ax) + by;
									if (ky >= h) okay = 0;
								}
							...
							e2->syloc = e2->btyloc = e1->eyloc = e1->bbyloc;
						}
					}
					else if (((ax > mx) && (mx > e1->btxloc)) || ((ax > mx) && (G_spline))) {
						li2 = li->next;
						while (li2) {
							if (okay == 0) break;
							if (!li2->node->anchordummy) {
...
void calc_edgearrow(GNODE v)
{
	...
	c = v->connection;
	if (c) {
		if (c->target && (c->edge->end == v))  gs_ide1511(c->edge);
		if (c->target2 && (c->edge2->end == v))  gs_ide1511(c->edge2);
	} a = v->succ;
	act_port = -1;
	while (a) {
		if ((a->kante->kantenart == 'l') || (a->kante->kantenart == 'r'));
	...
	act_port = -1;
	while (a) {
		if ((a->kante->kantenart == 'l') || (a->kante->kantenart == 'r')) gs_ide1511(a->kante);
		...
		assert(c && (c->target));
		assert((c->edge->end == v));
		if (c->edge->sxloc < c->edge->exloc) j = 8;
		else j = 7;
		a = v->succ;
		while (a) {
...
static void gs_ide1511(GEDGE e)
{
	...
	fval = (double)(e->eyloc - e->syloc) / (double)(e->exloc - e->sxloc);
	if (e->sxloc < e->exloc) {
		if (0.5 <= fval) e->orientation = 5;
		else if (-0.5 < fval) e->orientation = 7;
		else e->orientation = 2;
	}
	else {
		if (0.5 <= fval) e->orientation = 3;
		else if (-0.5 < fval) e->orientation = 8;
		else e->orientation = 6;
	} switch (e->orientation) {
	case 5: e->orientation2 = 3;
		break;
	case 7: e->orientation2 = 8;
		break;
	case 2: e->orientation2 = 6;
		break;
	case 6: e->orientation2 = 2;
		break;
	case 8: e->orientation2 = 7;
		break;
	case 3: e->orientation2 = 5;
		break;
	}
}
static void gs_ide1512(ADJEDGE a)
{
	...
	while (b) {
		if (port != b->kante->weights) break;
		e = b->kante;
		if (e->btyloc != e->syloc)  fval = (double)(e->btxloc - e->sxloc) / (double)(e->btyloc - e->syloc);
		else fval = (double)(e->bbxloc - e->sxloc) / (double)(e->bbyloc - e->syloc);
...
static void gs_ide1519(GNODE v)
{
	...
	while (v) {
		...
		c = v->connection;
		if (c) {
			if (backward_connection1(c)) gs_ide1520(c->edge);
			if (backward_connection2(c)) gs_ide1520(c->edge2);
		} li = v->pred;
		while (li) {
			gs_ide1520(li->kante);
			li = li->next;
...
static void gs_ide1520(GEDGE e)
{
	...
	switch (e->orientation) {
	case 0: break;
	case 1: e->orientation = 8;
		break;
	case 2: e->orientation = 6;
		break;
	case 3: e->orientation = 3;
		break;
	case 4: e->orientation = 7;
		break;
	case 5: e->orientation = 5;
		break;
	case 6: e->orientation = 2;
		break;
	case 7: e->orientation = 4;
		break;
	case 8: e->orientation = 1;
		break;
	} switch (e->orientation2) {
	case 0: break;
	case 1: e->orientation2 = 8;
		break;
	case 2: e->orientation2 = 6;
		break;
	case 3: e->orientation2 = 3;
		break;
	case 4: e->orientation2 = 7;
		break;
	case 5: e->orientation2 = 5;
		break;
	case 6: e->orientation2 = 2;
		break;
	case 7: e->orientation2 = 4;
		break;
	case 8: e->orientation2 = 1;
		break;
	}
}
...
static void gs_ide1524(void)
{
	...
	gs_ide1532 = 0;
	for (i = maxdepth + 1; i >= 0; i--) if (((layer[i]).succlist)) break;
	k = ((layer[i]).succlist);
	while (k) {
		if (k->node->yloc + k->node->height > gs_ide1532) gs_ide1532 = k->node->yloc + k->node->height;
		k = k->next;
...
static void gs_ide1522(GNODE v)
{
	...
	while (v) {
		...
		c = v->connection;
		if (c) {
			if (backward_connection1(c)) gs_ide1523(c->edge);
			if (backward_connection2(c)) gs_ide1523(c->edge2);
		} li = v->pred;
		while (li) {
			gs_ide1523(li->kante);
			li = li->next;
...
static void gs_ide1523(GEDGE e)
{
	...
	switch (e->orientation) {
	case 0: break;
	case 1: e->orientation = 4;
		break;
	case 2: e->orientation = 5;
		break;
	case 3: e->orientation = 6;
		break;
	case 4: e->orientation = 1;
		break;
	case 5: e->orientation = 2;
		break;
	case 6: e->orientation = 3;
		break;
	} switch (e->orientation2) {
	case 0: break;
	case 1: e->orientation2 = 4;
		break;
	case 2: e->orientation2 = 5;
		break;
	case 3: e->orientation2 = 6;
		break;
	case 4: e->orientation2 = 1;
		break;
	case 5: e->orientation2 = 2;
		break;
	case 6: e->orientation2 = 3;
		break;
	}
}
int maximal_xpos;
...
void calc_max_xy_pos(void)
{
	...
	v = nodelist;
	while (v) {
		if (v->xloc + v->width > maximal_xpos) maximal_xpos = v->xloc + v->width;
		if (v->yloc + v->height > maximal_ypos) maximal_ypos = v->yloc + v->height;
		v = v->next;
	} v = labellist;
	while (v) {
		if (v->xloc + v->width > maximal_xpos) maximal_xpos = v->xloc + v->width;
...
void statistics(void)
{
	...
	while (e) {
		if (e->invisible) st_nr_invis_edges++;
		e = e->next;
	}
}
