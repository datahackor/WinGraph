...
#include "timing.h"
#ifdef _DEBUG
static void gs_ide18(GNODE node);
static void gs_ide16(GEDGE edge);
...
void step1_main(void)
{
	...
	gs_ide29();
	gs_ide27();
	if (layout_flag == 3) gs_ide46();
	else if (layout_flag == 0) gs_ide37();
	else gs_ide33();
	if (edge_label_phase == 1) gs_ide2();
	if (fine_tune_layout == 1) gs_ide52();
...
static void gs_ide30(GNODE node)
{
	...
	else nodelistend = node->before;
	if (node->before) node->before->next = node->next;
	else nodelist = node->next;
	h = gs_ide44;
	hp = &gs_ide44;
	while (h) {
		if (h->refnum >= node->refnum) break;
		hp = &h->next;
		h = h->next;
	} *hp = node;
	if (h) node->before = h->before;
	else node->before = gs_ide45;
	node->next = h;
	if (h) h->before = node;
	else gs_ide45 = node;
...
static void gs_ide35(void)
{
	...
	while (node) {
		nxt_node = node->next;
		if (!node->pred) gs_ide30(node);
		else if (!node->succ) gs_ide28(node);
		node = nxt_node;
	} if (gs_ide44) {
		if (nodelist) nodelist->before = gs_ide45;
		((gs_ide45)->next) = nodelist;
		nodelist = gs_ide44;
		if (!nodelistend) nodelistend = gs_ide45;
	} if (gs_ide21) {
		if (nodelistend) nodelistend->next = gs_ide21;
		((gs_ide21)->before) = nodelistend;
		nodelistend = gs_ide22;
		if (!nodelist) nodelist = gs_ide21;
	}
}
...
void insert_anchor_edges(void)
{
	...
	edge = edgelist;
	while (edge) {
		if ((edge->anchor <= 64) && (edge->anchor > 0) && (!edge->invisible))  gs_ide34(edge);
		edge = edge->next;
	} edge = tmpedgelist;
	while (edge) {
		if ((edge->anchor <= 64) && (edge->anchor > 0) && (!edge->invisible))  gs_ide34(edge);
...
static void gs_ide34(GEDGE edge)
{
	...
	if (!c1) {
		...
		v->anchordummy = 1;
		h = tmpedgealloc(edge->linestyle, edge->thickness, edge->eclass, 200, edge->color, edge->labelcolor, 0, edge->arrowsize2, 0, edge->arrowstyle2, edge->arrowcolor1, edge->arrowcolor2, edge->horder);
		h->anchor = 66;
		h->start = edge->start;
		h->end = v;
		h->label = NULL;
	...
	h = tmpedgealloc(edge->linestyle, edge->thickness, edge->eclass, edge->priority, edge->color, edge->labelcolor, edge->arrowsize1, edge->arrowsize2, edge->arrowstyle1, edge->arrowstyle2, edge->arrowcolor1, edge->arrowcolor2, edge->horder);
...
void prepare_back_edges(void)
{
	...
	e = back_edge_list;
	while (e) {
		if (!e->kante->invisible) revert_edge(e->kante);
		else if (e->kante->labelnode) {
			a1 = e->kante->labelnode->succ;
			a2 = e->kante->labelnode->pred;
			if (a1)  if (!a1->kante->invisible)  revert_edge(a1->kante);
			if (a2)  if (!a2->kante->invisible)  revert_edge(a2->kante);
		} e = e->next;
	}
}
...
static void gs_ide27(void)
{
	...
	while (edge1) {
		...
		if (v) {
			edge = v->succ;
			if (!edge) invisible = 1;
			else invisible = edge->kante->invisible;
			edge = v->pred;
			if (!edge) invisible |= 1;
			else invisible |= edge->kante->invisible;
		}
		else if (!invisible) {
			if (G_displayel == 1)  v = create_labelnode(edge1->kante);
			else {
				v = gs_ide14(-1);
				v->invisible = 0;
			} v->level = edge1->kante->start->level;
			v->nhorder = edge1->kante->start->nhorder;
			edge = gs_ide15(v, edge1->kante->end, edge1->kante, 1);
			edge->kante->label = NULL;
			edge = gs_ide15(edge1->kante->start, v, edge1->kante, 0);
			edge->kante->label = NULL;
		...
		if (!invisible) {
			...
			if ((c1) && (c1->target) && (c1->target2))  connection_possible = 0;
			if ((c2) && (c2->target) && (c2->target2))  connection_possible = 0;
			if (gs_ide8(edge->kante->end, NULL, edge->kante->start))  connection_possible = 0;
			if (connection_possible) {
				...
				if (edge->kante->start->title[0]) FPRINTF(stderr, "%s", edge->kante->start->title);
				FPRINTF(stderr, " , ");
				if (edge->kante->end->title[0]) FPRINTF(stderr, "%s", edge->kante->end->title);
...
static void gs_ide29(void)
{
	...
	while (edge) {
		if (!edge->kante->invisible) {
			...
			if ((c2) && (c2->target) && (c2->target2))  connection_possible = 0;
			if (gs_ide8(edge->kante->end, NULL, edge->kante->start))  connection_possible = 0;
			if (connection_possible) { /*;ble = 0;
			arget) /+;R/SC;yy+NT ) && ((( c2 )->tar;*/ if (!c1) {
				c1 = connectalloc(edge->kante->start);
				c1->target = edge->kante->end;
				c1->edge = edge->kante;
			}
				...
				c2->target2 = edge->kante->start;
				c2->edge2 = edge->kante;
			} if ((edge->kante->start->level >= 0) && (edge->kante->end->level >= 0) && (edge->kante->start->level != edge->kante->end->level >= 0)) {
				if (!silent) {
					FPRINTF(stderr, "Nearedge connection (");
					if (edge->kante->start->title[0]) FPRINTF(stderr, "%s", edge->kante->start->title);
					FPRINTF(stderr, " , ");
				...
				edge->kante->end->level = edge->kante->start->level;
			}  if (edge->kante->end->level >= 0) {
				edge->kante->start->level = edge->kante->end->level;
			}  delete_adjedge(edge->kante);
			edge->kante->invisible = 0;
			}
			else if (!silent) {
...
static int gs_ide8(GNODE v, GNODE w, GNODE z)
{
	...
	c = v->connection;
	if (!c) return(0);
	if (c->target && (c->target != w))  r |= gs_ide8(c->target, v, z);
	if (c->target2 && (c->target2 != w))  r |= gs_ide8(c->target2, v, z);
	return(r);
} static long gs_ide0;
static int gs_ide1;
...
static void gs_ide33(void)
{
	...
	node = labellist;
	while (node) {
		if (!node->markiert) assert((0));
		node = node->next;
	}
#endif
	if (layout_flag == 12) return;
	if (layout_flag == TREE_LAYOUT) return;
	if (G_timelimit > 0)   if (test_timelimit(30)) {
		gs_wait_message('t');
		return;
	} depth1 = maxdepth;
	gs_wait_message('p');
...
static void gs_ide20(GNODE node)
{
	...
	assert((node));
	;
	if (node->markiert) return;
	node->markiert = 1;
	node->dfsnum = gs_ide0++;
	if (node->level >= 0) gs_ide1 = node->level;
	node->tiefe = gs_ide1;
	maxdepth = (gs_ide1 > maxdepth) ? gs_ide1 : maxdepth;
	if (node->connection) {
		if (node->connection->target)  gs_ide20(node->connection->target);
		if (node->connection->target2)  gs_ide20(node->connection->target2);
	} priority = 1;
	while (priority > -1) {
		priority = -1;
		edge = node->succ;
		while (edge) {
			...
			kn = edge->kante->end;
			if ((!kn->markiert) && (edge->kante->priority > priority))  priority = edge->kante->priority;
			edge = edge->next;
		} if (priority == -1) break;
		edge = node->succ;
		while (edge) {
			...
			if (!kn->markiert) {
				...
				if (kn == node) edge->kante->kantenart = 'S';
			} edge = edge->next;
		}
	}
}
...
static void gs_ide5(GNODE node)
{
	...
	maxdepth = (gs_ide1 > maxdepth) ? gs_ide1 : maxdepth;
	if (node->connection) {
		if (node->connection->target)  gs_ide5(node->connection->target);
		if (node->connection->target2)  gs_ide5(node->connection->target2);
	} priority = 1;
	while (priority > -1) {
		priority = -1;
		edge = node->succ;
...
static void gs_ide43(ADJEDGE edge, GNODE node, int priority)
{
	...
	if (edge->kante->priority != priority) return;
	kn = edge->kante->end;
	if (!kn->markiert) {
		gs_ide1++;
		gs_ide5(kn);
...
static int gs_ide49(GNODE node1, GNODE node2)
{
	...
	a = node1->pred;
	while (a) {
		if (a->kante->start == node1) a->kante->kantenart = 'S';
		else if (a->kante->start->markiert) {
			if (a->kante->start->tiefe >= result) result = a->kante->start->tiefe + 1;
		}   a = a->next;
	} c = node1->connection;
...
static void gs_ide50(GNODE node1, GNODE node2, int level)
{
	...
	node1->markiert = 1;
	c = node1->connection;
	if (c && c->target && (c->target != node2))  gs_ide50(c->target, node1, level);
	if (c && c->target2 && (c->target2 != node2))  gs_ide50(c->target2, node1, level);
}
...
static void gs_ide47(GNODE node1, GNODE node2)
{
	...
	a = node1->succ;
	while (a) {
		if ((gs_ide48(a->kante->end, a->kante->end) == 0) && (!a->kante->end->markiert))  gs_ide4(a->kante->end);
		a = a->next;
	} c = node1->connection;
	if (c && c->target && (c->target != node2))  gs_ide47(c->target, node1);
	if (c && c->target2 && (c->target2 != node2))  gs_ide47(c->target2, node1);
}
...
static int gs_ide48(GNODE node1, GNODE node2)
{
	...
	a = node1->pred;
	while (a) {
		if (!a->kante->start->markiert) result++;
		a = a->next;
	} c = node1->connection;
	if (c && c->target && (c->target != node2))  result += gs_ide48(c->target, node1);
	if (c && c->target2 && (c->target2 != node2))  result += gs_ide48(c->target2, node1);
	return(result);
...
static void gs_ide37(void)
{
	...
	v = nodelist;
	while (v) {
		if (!v->invisible) gs_ide3(v, &gs_ide25);
		v->tiefe = 1;
		v = v->next;
	} v = labellist;
	while (v) {
...
static void gs_ide41(GNODE node, long* dfsnum, GNLIST* open_sccp)
{
	...
	if (node->connection) {
		if (node->connection->target) {
			...
			if (kn->weightp && (kn->weights < node->weights))  node->weights = kn->weights;
	...
	while (edge) {
		...
		*open_sccp = h->next;
		...
		if (degree) {
			...
			while (kn) {
				if (kn->level >= 0) kn->tiefe = kn->level;
				else kn->tiefe = minlevel;
				maxdepth = (kn->tiefe > maxdepth) ? kn->tiefe : maxdepth;
#ifdef SCCDEBUG
				PRINTF("[%ld|%s] (%d) (max %d)\n", kn, (kn->title ? kn->title : "null"), kn->tiefe, maxdepth);
#endif
				kn = gs_ide23(&closed_scc_list);
			} return;
		} h = closed_scc_list;
...
#ifdef SCCDEBUG
		PRINTF("Revert Preds on [%d|%s] %d %d %d\n", actrev, (actrev->title ? actrev->title : "null"), minindeg, maxoutdeg, maxpreindeg);
#endif
	...
	while (count) {
		...
		while (h) {
			...
			if (v->markiert) {
				c = v->connection;
				if ((c) && c->edge) {
					if (!c->edge->start->markiert) count++;
					if (!c->edge->end->markiert) count++;
					c->edge->start->markiert = 1;
					c->edge->end->markiert = 1;
				} if ((c) && c->edge2) {
					if (!c->edge2->start->markiert) count++;
					if (!c->edge2->end->markiert) count++;
...
static int gs_ide39(GNODE v, GNODE w, int prio)
{
	...
	while (e) {
		if (e->kante->end->tiefe == MAXINT) {
			if (prio) degree += e->kante->priority;
			else degree++;
		} e = e->next;
	} c = v->connection;
	if (!c) return(degree);
	if (c->target && (w != c->target))  degree += gs_ide39(c->target, v, prio);
	if (c->target2 && (w != c->target2))  degree += gs_ide39(c->target2, v, prio);
	return(degree);
}
...
static int gs_ide40(GNODE v, GNODE w)
{
	...
	if (!c) return(degree);
	if (c->target && (w != c->target))  degree += gs_ide40(c->target, v);
	if (c->target2 && (w != c->target2))  degree += gs_ide40(c->target2, v);
	return(degree);
}
...
static int gs_ide38(GNODE v, GNODE w)
{
	...
	if (!c) return(degree);
	if (c->target && (w != c->target))  degree += gs_ide38(c->target, v);
	if (c->target2 && (w != c->target2))  degree += gs_ide38(c->target2, v);
	return(degree);
}
...
static void gs_ide36(GNODE v, GNODE w)
{
	...
	if (!c) return;
	if (c->target && (w != c->target))  gs_ide36(c->target, v);
	if (c->target2 && (w != c->target2))   gs_ide36(c->target2, v);
}
	...
	while (v) {
		...
		while (edge) {
			...
			if (edge->kante->label) {
				...
				vt = edge->kante->end;
				vl->tiefe = (edge->kante->start->tiefe + edge->kante->end->tiefe) / 2;
...
static void gs_ide52(void)
{
	...
	while (changed) {
		...
		while (v) {
			if (!v->connection) changed += gs_ide51(v, 0);
			v = v->next;
		}  v = labellist;
		while (v) {
			if (!v->connection) changed += gs_ide51(v, 1);
			v = v->next;
		}  count++;
		if (count >= 50) return;
	}
...
static int gs_ide51(GNODE v, int lab)
{
	...
	nodelevel = v->tiefe;
	if ((!v->succ) || (!v->pred)) delta = 1;
	else delta = 0;
	edge = v->pred;
	while (edge) {
		nr_edges += edge->kante->priority;
		nr_redges++;
		if ((((((edge->kante)->start)))->tiefe) != nodelevel) leveldiff += (edge->kante->priority * (edge->kante->start->tiefe - nodelevel + delta));
		edge = edge->next;
	} edge = v->succ;
	while (edge) {
		nr_edges += edge->kante->priority;
		nr_redges++;
		if (edge->kante->end->tiefe != nodelevel) leveldiff += (edge->kante->priority * (edge->kante->end->tiefe - nodelevel - delta));
		edge = edge->next;
	} if (nr_redges == 0) {
		v->tiefe = 0;
		return(0);
...
#ifdef TUNEDEBUG
	PRINTF("Wish %sto move \"%s\" from level %d to level %d\n", (changed ? "" : "not "), (v->title ? v->title : "(null)"), v->tiefe, nodelevel);
#endif
	if (!changed) return(0);
	if ((layout_flag == 3) || (layout_flag == 0)) {
		if (near_edge_layout) delta = 0;
		else delta = 1;
		if (nodelevel > v->tiefe) {
			edge = v->succ;
			while (edge) {
				if ((edge->kante->end->tiefe > v->tiefe) && (nodelevel >= (((((edge->kante)->end)))->tiefe))) nodelevel = edge->kante->end->tiefe - delta;
				edge = edge->next;
			} edge = v->pred;
			while (edge) {
				if ((edge->kante->start->tiefe > v->tiefe) && (nodelevel >= edge->kante->start->tiefe)) nodelevel = edge->kante->start->tiefe - delta;
				edge = edge->next;
			} if (nodelevel <= v->tiefe) return(0);
		} if (nodelevel < v->tiefe) {
			edge = v->succ;
			while (edge) {
				if ((edge->kante->end->tiefe < v->tiefe) && (nodelevel <= edge->kante->end->tiefe)) nodelevel = edge->kante->end->tiefe + delta;
		...
		while (edge) {
			if (edge->kante->kantenart != 'R') {
				if ((nodelevel < edge->kante->start->tiefe) && (v->tiefe >= edge->kante->start->tiefe)) return(0);
			} edge = edge->next;
		} edge = v->succ;
		while (edge) {
			if (edge->kante->kantenart != 'R') {
				if ((nodelevel > edge->kante->end->tiefe) && (v->tiefe <= edge->kante->end->tiefe)) return(0);
			} edge = edge->next;
		}
	}
	else {
		edge = v->pred;
		while (edge) {
			if (nodelevel == edge->kante->start->tiefe) return(0);
			edge = edge->next;
		} edge = v->succ;
		while (edge) {
			if (nodelevel == edge->kante->end->tiefe) return(0);
			edge = edge->next;
		}
	} delta = 0;
	edge = v->pred;
	while (edge) {
		...
		if (nodelevel == hh->tiefe) {
			delta++;
			if (hh->connection) return(0);
			hdelta = 0;
			hedge = hh->pred;
			while (hedge) {
				if (hh->tiefe == hedge->kante->start->tiefe) hdelta++;
				hedge = hedge->next;
			} hedge = hh->succ;
			while (hedge) {
				if (hh->tiefe == hedge->kante->end->tiefe) hdelta++;
				hedge = hedge->next;
...
#endif
	if (v->tiefe != nodelevel) changed = 1;
	v->tiefe = nodelevel;
	return(changed);
}
...
static void gs_ide11(void)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		while (n) {
			...
			c = node->connection;
			if (c) {
				if (backward_connection1(c))  backward_connection = 1;
				if (backward_connection2(c)) backward_connection = 1;
				if (forward_connection1(c)) forward_connection = 1;
				if (forward_connection2(c))  forward_connection = 1;
			} if ((forward_connection) && (!backward_connection) && (n->node->markiert == 0))  gs_ide6(node, node, NULL);
			...
			if (node->indegree > maxindeg)  maxindeg = node->indegree;
			n = n->next;
		}
	}
}
...
static void gs_ide6(GNODE v, GNODE w, GNODE predw)
{
	...
	if (!c) return;
	if (c->target && (c->target != predw))  gs_ide6(v, c->target, w);
	if (c->target2 && (c->target2 != predw))  gs_ide6(v, c->target2, w);
}
...
static void gs_ide10(GNODE node, ADJEDGE edge, int level)
{
	...
	if (edgelen < 0) {
		...
		gs_ide10(e1->kante->start, e1, e1->kante->start->tiefe);
	}
	else if (edgelen == 0) {
		if (edge->kante->kantenart == 'R') {
			e1 = revert_edge(edge->kante);
			...
			gs_ide10(e2->kante->start, e2, e2->kante->start->tiefe);
			gs_ide10(e3->kante->start, e3, e3->kante->start->tiefe);
			delete_adjedge(edge->kante);
		}
		else {
			c1 = node->connection;
			...
			if ((c2) && (c2->target) && (c2->target2))  connection_possible = 0;
			if (gs_ide8(edge->kante->end, NULL, edge->kante->start))  connection_possible = 0;
			/*;) && ((( c1 )->target) /+;R/SC;yy+NT )
				sible = 0;;*/ if (connection_possible) {
					if (!c1) {
					...
					if (level <= maxdepth)  d1 = gs_ide14(level + 1);
					else if (level > 0)   d1 = gs_ide14(level - 1);
					else {
						assert((0));
					} 
					d1->nhorder = edge->kante->horder;
					e1 = gs_ide15(node, d1, edge->kante, 0);
					e2 = gs_ide15(d1, edge->kante->end, edge->kante, 1);
					e2->kante->label = edge->kante->label;
					gs_ide10(e1->kante->start, e1, e1->kante->start->tiefe);
					gs_ide10(e2->kante->start, e2, e2->kante->start->tiefe);
					delete_adjedge(edge->kante);
		...
		if (!lab_set) e1->kante->label = edge->kante->label;
		delete_adjedge(edge->kante);
	}
}
...
ADJEDGE revert_edge(GEDGE edge)
{
	...
	edge->start = edge->end;
	edge->end = h;
	if (edge->kantenart == 'R') edge->kantenart = 'U';
	else if (edge->kantenart == 'S') edge->kantenart = 'S';
	else edge->kantenart = 'R';
	hh = edge->arrowsize1;
...
static ADJEDGE gs_ide15(GNODE start, GNODE end, GEDGE edge, int arrow)
{
	...
	h->end = end;
	switch (arrow) {
	case 0: h->arrowstyle1 = 0;
		h->arrowsize1 = 0;
		h->label = NULL;
		break;
	case 1: h->arrowstyle2 = 0;
		h->arrowsize2 = 0;
		h->label = NULL;
		break;
	case 2: h->arrowstyle1 = 0;
		h->arrowsize1 = 0;
		h->label = NULL;
		h->arrowstyle2 = 0;
		h->arrowsize2 = 0;
		h->label = NULL;
		break;
	}   if (edge->kantenart == 'S') h->kantenart = 'U';
	else h->kantenart = edge->kantenart;
	if (start == end) h->kantenart = 'S';
	create_adjedge(h);
	return(h->start->succ);
}
...

{
	...
	v->before = NULL;
	v->next = dummylist;
	if (dummylist) dummylist->before = v;
	dummylist = v;
	dummyanz++;
	if (t < 0) return(v);
	hl = tmpnodelist_alloc();
...
static void gs_ide9(ADJEDGE edge)
{
	...
	while (l) {
		...
		} if (f1->start != f2->start) tide = 0;
		if (f1->end != f2->end) tide = 0;
		if (f1->linestyle != f2->linestyle) ide = 0;
		if (f1->thickness > f2->thickness) ide = 0;
		if (f1->priority > f2->priority) ide = 0;
		if (f1->horder != f2->horder) ide = 0;
		if (f1->eclass != f2->eclass) ide = 0;
		if (f1->color != f2->color) ide = 0;
		if (f1->arrowsize1 != f2->arrowsize1) aside1 = 0;
		if (f1->arrowsize2 != f2->arrowsize2) aside2 = 0;
		if (f1->arrowstyle1 != f2->arrowstyle1) aside1 = 0;
		if (f1->arrowstyle2 != f2->arrowstyle2) aside2 = 0;
		if (f1->arrowcolor1 != f2->arrowcolor1) aside1 = 0;
		if (f1->arrowcolor2 != f2->arrowcolor2) aside2 = 0;
		if (f1->anchor != f2->anchor) ide = 0;
...
void calc_number_reversions(void)
{
	...
	while (v) {
		e = v->pred;
		while (e) {
			if (e->kante->kantenart == 'R') number_reversions++;
			e = e->next;
		} v = v->next;
	} v = labellist;
	while (v) {
...
void db_output_adjacencies(void)
{
	...
	while (node) {
		...
		edge = node->succ;
		while (edge) {
			PRINTF("|%s(%ld) ", edge->kante->end->title, edge->kante->end);
			edge = edge->next;
		}
		PRINTF("\nPreds:");
		edge = node->pred;
		while (edge) {
			PRINTF("|%s(%ld) ", edge->kante->start->title, edge->kante->start);
			edge = edge->next;
		} node = node->next;
	}
	node = labellist;
...
void db_output_adjacency(GNODE node, int f)
{
	...
	if (f != 1)
	{
		...
		while (edge)
		{
			...
			PRINTF("(%ld)%s-", edge->kante->start, gtitle(edge->kante->start));
			PRINTF("(%ld)%s ", edge->kante->end, gtitle(edge->kante->end));
			edge = edge->next;
		}
	}
	if (f != 0)
...
static void gs_ide19(char* fn)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		while (li) {
			node = li->node;
			FPRINTF(f, "\nnode: { title: \"%ld\" ", node);
			if (node->title && (node->title[0])) FPRINTF(f, "label: \"%s\" ", node->title);
			FPRINTF(f, "level: %d ", i);
			...
			while (edge) {
				FPRINTF(f, "edge: { ");
				FPRINTF(f, "sourcename: \"%ld\" ", edge->kante->start);
