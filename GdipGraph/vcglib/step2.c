...
/// ([\S])\(([a-z0-9]+)->([a-z0-9]+)->([a-z0-9]+)\) $1$2->$3->$4
/// ([\S])\(([a-z0-9]+)->([a-z0-9]+)->([a-z0-9]+)->([a-z0-9]+)\) $1$2->$3->$4->$5
/// ([\S])\(([a-z0-9]+)->([a-z0-9]+)->([a-z0-9]+)->([a-z0-9]+)->([a-z0-9]+)\) $1->$2->$3->$4->$5->$6
 ...
 */
 ...
 //#ifndef lint
 //static char* gs_ide530 = "$Id: step2.c,v 3.12 1995/02/08 11:11:14 sander Exp $";
...
void step2_main(void)
{
	...
	if (max_nodes_per_layer + 2 > gs_ide573) {
		if (gs_ide577) free(gs_ide577);
		if (gs_ide569) free(gs_ide569);
		gs_ide577 = (GNODE*)malloc((max_nodes_per_layer + 2) * sizeof(GNODE));
		gs_ide569 = (GNODE*)malloc((max_nodes_per_layer + 2) * sizeof(GNODE));
		if ((!gs_ide577) || (!gs_ide569))  Fatal_error("memory exhausted", "");
	...
	if (i + 2 > gs_ide572)
	{
		if (gs_ide504) free(gs_ide504);
		if (gs_ide505) free(gs_ide505);
...
static void gs_ide519(void)
{
	...
	if (maxdepth + 2 > gs_ide574)
	{
		if (gs_ide580)
			free(gs_ide580);
		gs_ide580 = (DEPTH*)malloc((maxdepth + 2) * sizeof(struct depth_entry));
		if (!gs_ide580)
			Fatal_error("memory exhausted", "");
...
static void gs_ide581()
{
	...
	for (i = 1; i <= maxdepth + 1; i++)
	{
		...
		while (li) {
			...
			if (node->bary > maxbary) maxbary = node->bary;
			li = li->next;
		} maxbary = maxbary + 1.0;
		...
		while (li) {
			...
			while (a) {
				...
				if (e->weights) {
					node = a->kante->end;
					node->bary = (double)li->node->position + node->bary / maxbary;
				} a = a->next;
			} li = li->next;
...
static void gs_ide556()
{
	...
	for (i = 0; i <= maxdepth + 1; i++)
	{
		...
		while (li) {
			node = li->node;
			if (node->nhorder >= 0) reorder_necessary = 1;
			if (node->nhorder > gs_ide547) gs_ide547 = node->nhorder;
			li = li->next;
		} ((gs_ide580[i]).resort_necessary) = ((layer[i]).resort_necessary) = reorder_necessary;
	}
}
...
static void gs_ide546(GNODE node, int i)
{
	...
	if (node->bary)
		return;
	if (node->nhorder >= 0)  node->bary = (double)i * (double)(gs_ide547 + 1) + (double)node->nhorder;
	else node->bary = (double)i * (double)(gs_ide547 + 1);
	e = node->succ;
	while (e) {
...
static void gs_ide507(GNODE n)
{
	...
	if (!gs_ide584) gs_ide584 = d;
	if (gs_ide585) ((gs_ide585)->succ) = d;
	gs_ide585 = d;
	gs_ide575++;
}
...
static void gs_ide506(GNODE n)
{
	...
	if (!gs_ide544) gs_ide544 = d;
	if (gs_ide545) ((gs_ide545)->succ) = d;
	gs_ide545 = d;
	gs_ide571++;
}
...
static void gs_ide522(DLLIST x)
{
	...
	else gs_ide584 = x->succ;
	if (x->succ) x->succ->pred = x->pred;
	else gs_ide585 = x->pred;
	dllist_free(x);
	gs_ide575--;
}
...
static void gs_ide521(DLLIST x)
{
	...
	;
	if (x->pred) x->pred->succ = x->succ;
	else gs_ide544 = x->succ;
	if (x->succ) x->succ->pred = x->pred;
	else gs_ide545 = x->pred;
	dllist_free(x);
	gs_ide571--;
}
...
static int gs_ide525(GNODE C, GNODE D)
{
	...
	while (a) {
		...
		h = actlistC;
		while (h) {
			if (n->position < h->node->position) Sum1++;
			else if (n->position > h->node->position) Sum2++;
			h = h->succ;
		} a = a->next;
	}
...
static int gs_ide540(int level, int nearedges)
{
	...
	while (vl2) {
		...
		if (vl1) {
			...
			possible = 1;
			if (nearedges) {
				if (n1->connection || n2->connection) possible = 0;
...
static int gs_ide513(GNODE v1, GNODE v2, int dir)
{
	...
	GNODE n1, n2;
	;
	if (gs_ide535(v1)) return(0);
	if (gs_ide535(v2)) return(0);
...
static void gs_ide583(GNODE v)
{
	...
	while (a1)
	{
		a2 = v->succ;
		while (a2) {
			if (gs_ide513(a1->kante->end, a2->kante->end, 'S')) {
				gs_ide523(a1->kante->end, a2->kante->end, 'S');
			} a2 = a2->next;
		} a1 = a1->next;
	}
	a1 = v->pred;
	while (a1)
	{
		...
		while (a2)
		{
			if (gs_ide513(a1->kante->start, a2->kante->start, 'P')) {
				gs_ide523(a1->kante->start, a2->kante->start, 'P');
			} a2 = a2->next;
		} a1 = a1->next;
	}
...
static int gs_ide561(int i)
{
	...
	case 2:
		for (j = 0; j < ((layer[i + 1]).anz); j++)
			((gs_ide577[j])->bary) = gs_ide554(gs_ide577[j]) + gs_ide555(gs_ide577[j]) / 10000.0;
		break;
	case 3:
		for (j = 0; j < ((layer[i + 1]).anz); j++)
			((gs_ide577[j])->bary) = gs_ide555(gs_ide577[j]) + gs_ide554(gs_ide577[j]) / 10000.0;
		break;
	}
	...
	if (c <= ((gs_ide580[i]).cross)) {
		((gs_ide580[i]).cross) = c;
		if (i < maxdepth) ((gs_ide580[i + 1]).cross) = gs_ide536(i + 1);
		return(1);
	}
	gs_ide565(i + 1);
	return(0);
...
static int gs_ide564(int i)
{
	...
	case 2:
		for (j = 0; j < ((layer[i]).anz); j++)
			((gs_ide577[j])->bary) = gs_ide578(gs_ide577[j]) + gs_ide579(gs_ide577[j]) / 10000.0;
		break;
	case 3:
		for (j = 0; j < ((layer[i]).anz); j++)
			((gs_ide577[j])->bary) = gs_ide579(gs_ide577[j]) + gs_ide578(gs_ide577[j]) / 10000.0;
		break;
	}
	quicksort_sort_array(((layer[i]).anz));
	gs_ide570(i);
...
static void gs_ide502(void)
{
	...
	if (gs_ide553 <= maxdepth)
		for (i = gs_ide553; i <= maxdepth; i++) {
			...
			if (gs_ide520(((layer[i]).anz))) {
				gs_ide509(i);
				if (((layer[i]).resort_necessary)) gs_ide508(i);
				if (i > 0) ((gs_ide580[i - 1]).cross) = gs_ide536(i - 1);
				if (i <= maxdepth) ((gs_ide580[i]).cross) = gs_ide536(i);
				gs_ide563(i);
				cross = gs_ide528();
				if (cross < nr_crossings) {
#ifdef CHECK_CROSSING
	...
	for (i = 0; (i < gs_ide553) && (i <= maxdepth); i++)
	{
		...
		case 0:
			...
			break;
		case 1:
			for (j = 0; j < ((layer[i]).anz); j++) ((gs_ide577[j])->bary) = gs_ide555(gs_ide577[j]);
			break;
		case 2:
			for (j = 0; j < ((layer[i]).anz); j++) ((gs_ide577[j])->bary) = gs_ide554(gs_ide577[j]) + gs_ide555(gs_ide577[j]) / 10000.0;
			break;
		case 3:
			for (j = 0; j < ((layer[i]).anz); j++) ((gs_ide577[j])->bary) = gs_ide555(gs_ide577[j]) + gs_ide554(gs_ide577[j]) / 10000.0;
			break;
		} quicksort_sort_array(((layer[i]).anz));
		if (gs_ide520(((layer[i]).anz))) {
			gs_ide509(i);
...
static void gs_ide503(void)
{
	...
	if (gs_ide553 > 0)
		for (i = gs_ide553; i > 0; i--) {
			...
			switch (crossing_heuristics) {
			case 0:
				for (j = 0; j < ((layer[i]).anz); j++) ((gs_ide577[j])->bary) = gs_ide554(gs_ide577[j]);
				break;
			case 1:
				for (j = 0; j < ((layer[i]).anz); j++) ((gs_ide577[j])->bary) = gs_ide555(gs_ide577[j]);
				break;
			case 2:
				...
				break;
			case 3:
				for (j = 0; j < ((layer[i]).anz); j++) ((gs_ide577[j])->bary) = gs_ide579(gs_ide577[j]) + gs_ide554(gs_ide577[j]) / 10000.0;
				break;
			}
			quicksort_sort_array(((layer[i]).anz));
			if (gs_ide520(((layer[i]).anz))) {
...
static int gs_ide520(int siz)
{
	...
	for (j = 0; j < siz - 1; j++) {
		if (((gs_ide577[j])->bary) == ((gs_ide577[j + 1])->bary)) {
			...
			h = gs_ide577[j];
			for (k = j; k > start_region; k--) gs_ide577[k] = gs_ide577[k - 1];
...
static double gs_ide578(GNODE node)
{
	...
	assert((node));
	;
	if (node->outdegree == 0) return(0.0);
	Sum = 0;
	w = node->succ;
	while (w) {
		...
		Sum -= w->kante->anchor;
		w = w->next;
	} return (((double)Sum) / ((double)(256 * node->outdegree)));
}
...
static double gs_ide554(GNODE node)
{
	...
	while (w) {
		...
		Sum += w->kante->anchor;
		w = w->next;
	} return (((double)Sum) / ((double)(256 * node->indegree)));
}
...
static double gs_ide579(GNODE node)
{
	...
	case 0: return(0.0);
	case 1: return((double)node->succ->kante->end->position - (double)node->succ->kante->anchor / 256.0);
	case 2:   w = node->succ;
	...
	if (i % 2) return((double)((gs_ide569[i / 2])->position));
	leftpart = ((gs_ide569[i / 2 - 1])->position) - ((gs_ide569[0])->position);
	rightpart = ((gs_ide569[i - 1])->position) - ((gs_ide569[i / 2])->position);
	return (((double)(((gs_ide569[i / 2 - 1])->position) * rightpart + ((gs_ide569[i / 2])->position) * leftpart)) / ((double)(leftpart + rightpart)));
}
...
static double gs_ide555(GNODE node)
{
	...
	case 0: return(0.0);
	case 1: return((double)node->pred->kante->start->position + (double)node->pred->kante->anchor / 256.0);
	case 2:   w = node->pred;
...
static void gs_ide549(int l, int r)
{
	...
	i = l;
	j = r + 1;
	if (v->bary == 0.0) j = l + k / 2;
	else {
		while (i < j) {
			do j--;
			while ((((gs_ide577[j])->bary) != 0.0) && (((gs_ide577[j])->bary) > v->bary));
			do i++;
			while ((i <= j) && (((gs_ide577[i])->bary) != 0.0) && (((gs_ide577[i])->bary) < v->bary));
			if (i < j) exchange(gs_ide577[j], gs_ide577[i]);
		};
	} exchange(gs_ide577[l], gs_ide577[j]);
	if (l < j - 1) gs_ide549(l, j - 1);
	if (j + 1 < r) gs_ide549(j + 1, r);
}
#endif
...
static void gs_ide508(int i)
{
	...
	j = 0;
	while (hn) {
		if (hn->node->nhorder >= 0)  hn->node = gs_ide577[j++];
		hn = hn->next;
	}
}
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		if (((l1[i]).anz) == ((l2[i]).anz)) {
			...
			while (h2) {
				...
				h1->node = h2->node;
				h2 = h2->next;
				if (h2 && !h1->next)  h1->next = tmpnodelist_alloc();
				h1 = h1->next;
			} ((l1[i]).anz) = ((l2[i]).anz);
		}
	}
...
static void gs_ide532(void)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		while (hl) {
			...
			forward_conn = 0;
			if (c) {
				if (forward_connection1(c)) forward_conn = 1;
				if (forward_connection2(c))  forward_conn = 1;
...
static void gs_ide512(int level, GNODE node)
{
	...
	if (forward_connection1(c)) gs_ide537(c->target, node);
	if (forward_connection2(c)) gs_ide566(c->target2, node);
	gs_ide533(level, node);
	clr = 0;
	if (level > 0) clr += gs_ide536(level - 1);
	if (level <= maxdepth) clr += gs_ide536(level);
	*gs_ide534 = gs_ide548;
	((gs_ide548)->next) = gs_ide524;
	gs_ide538 = gs_ide539 = gs_ide567 = gs_ide568 = NULL;
	if (forward_connection1(c)) gs_ide566(c->target, node);
	if (forward_connection2(c)) gs_ide537(c->target2, node);
	gs_ide533(level, node);
	crl = 0;
	if (level > 0) crl += gs_ide536(level - 1);
	if (level <= maxdepth) crl += gs_ide536(level);
	if (crl <= clr) return;
	*gs_ide534 = gs_ide548;
	((gs_ide548)->next) = gs_ide524;
	gs_ide538 = gs_ide539 = gs_ide567 = gs_ide568 = NULL;
...
static void gs_ide576(GNODE v)
{
	...
	qsort(gs_ide505, v->indegree, sizeof(GNODE), (int (*) (const void*, const void*))gs_ide516);
	i = 0;
	a = v->pred;
	while (a) {
		a->kante = gs_ide505[i++];
	...
	qsort(gs_ide505, v->outdegree, sizeof(GNODE), (int (*) (const void*, const void*))gs_ide517);
	i = 0;
	a = v->succ;
	while (a) {
		a->kante = gs_ide505[i++];
...
static int gs_ide516(const GEDGE* a, const GEDGE* b)
{
	if (((((*a)->start))->position) > ((((*b)->start))->position)) return(1);
	if (((((*a)->start))->position) < ((((*b)->start))->position)) return(-1);
	return(0);
}
...
static int gs_ide517(const GEDGE* a, const GEDGE* b)
{
	if (((((*a)->end))->position) > ((((*b)->end))->position)) return(1);
	if (((((*a)->end))->position) < ((((*b)->end))->position)) return(-1);
	return(0);
}
#ifdef _DEBUG
...
void db_output_layer(int i)
{
	...
	li = ((gs_ide580[i]).succlist);
	while (li) {
		if (li->node->title[0]) PRINTF("%s[", li->node->title);
		else PRINTF("?[");
		li2 = li->node->pred;
		while (li2) {
			if (li2->kante->start->title[0]) PRINTF("%s,", li2->kante->start->title);
			else PRINTF("?,");
			li2 = li2->next;
		} PRINTF("][");
		li2 = li->node->succ;
		while (li2) {
			if ((((li2->kante->end))->title)[0]) PRINTF("%s,", li2->kante->end->title);
			else PRINTF("?,");
			li2 = li2->next;
		} PRINTF("]");
		PRINTF("b(%f)p(%d) ", li->node->bary, li->node->position);
		li = li->next;
	} PRINTF("\n");
}
#endif
...
int db_check_proper(GNODE v, int level)
{
	...
	while (li)
	{
		...
		{
			PRINTF("Succedge (%s,%s) depth %d (%d expected)\n", st, tt, li->kante->start->tiefe, t - 1);
			break;
		}
		li = li->next;
	}
