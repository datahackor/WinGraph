...
int vcg_main(int argc, char* argv[]) 
{
	...
	SPRINTF(short_banner, "USAAR Visualization Tool VCG/XVCG %s %s", version_str, revision_str);
	...
	{  /* Get arguments from the command line */
		...
		if (!scanOptions(argc, argv)) 
			print_basic_help(); /* and exit */
	}
...
void parse_part(void) 
{
	...
	else 
	{
		...
		if (yyin == NULL) 
			Fatal_error("Cannot open", Dataname);
...
void display_part(void)
{
	...
	if (remove_input_file) _unlink(Dataname);
}
