...
#undef DRAWDEBUG
#ifdef DRAWDEBUG
extern void debug_display_part(void);
extern void display_part(void);
...
void step3_main(void)
{
	...
	gs_ide1022();
	gs_ide1021();
	if (straight_phase == 1) gs_ide1048();
	gs_ide1043();
	gs_ide1045();
	gs_ide1041();
	calc_all_ports(1);
...
void alloc_levelshift(void)
{
	...
	if (sizeof(GNLIST) < sizeof(int)) Fatal_error("Assertion 2 about structs is wrong.", "");
#ifdef TESTLAYOUT
	PRINTF("Maximal nodes per layer: %d\n", max_nodes_per_layer);
	PRINTF("Maximal layers: %d\n", maxdepth);
#endif
	if (max_nodes_per_layer + 2 > gs_ide1046) {
		if (gs_ide1025) free(gs_ide1025);
		...
		tpred_connection1 = (GNLIST*)malloc((max_nodes_per_layer + 2) * sizeof(GNLIST));
		tpred_connection2 = (GNLIST*)malloc((max_nodes_per_layer + 2) * sizeof(GNLIST));
		gs_ide1026 = (int*)tpred_connection1;
		gs_ide1047 = (GNODE*)tpred_connection2;
		if ((!gs_ide1025) || (!gs_ide1026) || (!gs_ide1047)) Fatal_error("memory exhausted", "");
	...
	while (v) {
		if (v->anchordummy) {
			...
			if (manhatten_edges == 1) {
				if ((h <= 1) && (hh == 0)) v->width = 0;
				if ((h == 0) && (hh <= 1)) v->width = 0;
			}
		}
		else {
			v->width = 0;
...
static void gs_ide1017(void)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		while (li) {
			v = li->node;
			if ((v->width == 0) && (v->height == 0)) v->xloc = dxralign(actxpos + v->width / 2) - v->width / 2;
			else (v->xloc) = xralign(actxpos + v->width / 2) - v->width / 2;
			v->yloc = actypos;
			actxpos = v->xloc + v->width + G_xspace;
			if (maxboxheight < v->height)  maxboxheight = v->height;
			li = li->next;
		} if (G_yalign == 1) {
			li = ((layer[i]).succlist);
			while (li) {
...
static void gs_ide1001(void)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		...
		if (li) {
			...
			shift_value = (li->node->xloc + (li->node->width)) / 2;
...
static void gs_ide1043(void)
{
	...
	for (i = 0; i <= maxdepth + 1; i++) {
		li = ((layer[i]).succlist);
		while (li) {
			if (li->node->xloc < minx) minx = (li->node->xloc);
			if (li->node->yloc < miny) miny = (li->node->yloc);
			li = li->next;
		}
	} minx = minx - G_xbase;
	miny = miny - G_ybase;
...
static void gs_ide1042(int t)
{
	...
	while (intermixed_part_found) {
		...
		for (i = 0; i <= maxdepth + 1; i++) {
			...
			while (li) {
				if (li->node->markiert == 0) {
					if (li->next && li->next->node->markiert) {
						node = li->node;
						intermixed_part_found = 1;
						break;
					}
...
static void gs_ide1044(GNODE node, int i)
{
	...
	node->xloc -= i;
	if (node->connection) {
		if (node->connection->target) gs_ide1044(node->connection->target, i);
		if (node->connection->target2) gs_ide1044(node->connection->target2, i);
	} e = node->succ;
	while (e) {
...
static void gs_ide1027(GNODE node)
{
	ADJEDGE e;
	;
	if (node->markiert == 2) return;
	node->markiert = 2;
	if (node->xloc + node->width > gs_ide1015)  gs_ide1015 = node->xloc + node->width;
	if (node->connection) {
		if (node->connection->target) gs_ide1027(node->connection->target);
		if (node->connection->target2) gs_ide1027(node->connection->target2);
	} e = node->succ;
	while (e) {
...
static void gs_ide1051(GNODE v)
{
	...
	if (v->xloc < TMINX(layer[level])) {
		...
		li = li->next;
		if (li) {
			((layer[level]).actx) = v->xloc - G_xspace - (li->node->xloc) - (li->node->width);
		}
		else ((layer[level]).actx) = v->xloc - G_xbase;
	} c = v->connection;
	if (c) {
		if (c->target) {
			if (c->target->xloc > v->xloc) weight = 1;
			else weight = (c->edge->priority) * layout_nearfactor;
			if (weight != 0)  gs_ide1051(c->target);
		} if (c->target2) {
			if (c->target2->xloc > v->xloc) weight = 1;
			else weight = (c->edge2->priority) * layout_nearfactor;
			if (weight != 0)  gs_ide1051(c->target2);
		}
	} e = v->succ;
	while (e) {
...
static void gs_ide1022(void)
{
	...
	while (1) {
		...
		if (count >= max_mediumshifts) {
			gs_wait_message('t');
			break;
		} if (G_timelimit > 0) if (test_timelimit(85)) {
			gs_wait_message('t');
			break;
...
static int gs_ide1010(int first)
{
	...
	if (first) {
		for (i = 1; i <= maxdepth + 1; i++) {
			...
			for (i = 0; i <= maxdepth + 1; i++)  layer_changed += gs_ide1005(i);
		}
...
static int gs_ide1036(int i, int dir)
{
	...
	while (li) {
		...
		if ((sign < 0) && (gs_ide1025[j] >= 0)) gs_ide1026[j] = 1;
		else gs_ide1026[j] = MAXINT;
		if (gs_ide1025[j] < 0) sign = -1;
		else sign = 1;
		j++;
		li = (li->next);
	} gs_ide1026[0] = 1;
...
static int gs_ide1050(int i, int dir)
{
	...
	while (li) {
		...
		if (li->next) {
			w = (li->next->node);
			if (!touching(v, w))  gs_ide1026[j + 1] = 1;
		}    j++;
		li = (li->next);
	} 
	changed = 1;
	while (changed) {
		...
		while (li) {
			...
			if (li->next && (gs_ide1026[j + 1] != MAXINT)) {
				...
				w = (li->next->node);
				if (touching(v, w)) {
					if ((sum >= 0) && (gs_ide1026[j + 1] >= 0) && (sum >= gs_ide1026[j + 1])) {
...
static int gs_ide1005(int i)
{
	...
	while (changed) {
		...
		while (li) {
			...
			if (li->next) {
				dist = G_xspace;
				if ((v->width == 0) && (v->height == 0)) dist = G_dspace;
				w = (li->next->node);
				if ((w->width == 0) && (w->height == 0)) dist = G_dspace;
...
static void gs_ide1048(void)
{
	...
	while (changed) {
		...
		if (G_timelimit > 0) if (test_timelimit(90)) {
			gs_wait_message('t');
			return;
		} 
		changed = 0;
...
static int gs_ide1008(GNODE v)
{
	...
	if (possible) {
		...
		while (w && (gs_ide1018(w))) {
			...
			h2 = w->xloc + diff;
			if (diff < 0) {
				if (w->width == 0)  h2 = dxlalign(h2 + w->width / 2) - w->width / 2;
				else h2 = xlalign(h2 + w->width / 2) - w->width / 2;
				w->xloc = h2;
				allzero = 0;
			}
			else if (diff > 0) {
...
static int gs_ide1014(GNODE sw, int sxpos, int dir)
{
	...
	while (w && (gs_ide1018(w))) {
		...
		if (diff < 0) {
			...
			if (li) {
				gs_ide1030 = li->node;
				if ((w->width == 0) || (((gs_ide1030)->width) == 0))  h1 = ((gs_ide1030)->xloc) - G_dspace - w->width;
				else h1 = ((gs_ide1030)->xloc) - G_xspace - w->width;
				if (w->width == 0)  h2 = dxralign(h2 + w->width / 2) - w->width / 2;
				else h2 = xralign(h2 + w->width / 2) - w->width / 2;
...
static void gs_ide1049(GNODE sw, int sxpos, int dir)
{
	...
	while (w && (gs_ide1018(w))) {
		...
		diff = sxpos - w->xloc - w->width / 2;
		h2 = w->xloc + diff;
		if (diff < 0) li = ((layer[w->tiefe]).predlist);
		else li = ((layer[w->tiefe]).succlist);
		while (li) {
			if (li->node == w) break;
			li = li->next;
		} 
	...
	for (j = 0; j < ((layer[i]).anz); j++) {
		...
		if (diff < 0) {
			...
			if (lnode) {
				...
				{
					if (node->xloc < lnode->xloc + lnode->width + G_dspace) 
						node->xloc = lnode->xloc + lnode->width + G_dspace;
				}
				else 
				{
					if (node->xloc < lnode->xloc + lnode->width + G_xspace) 
						node->xloc = lnode->xloc + lnode->width + G_xspace;
				}
			} 
			if ((node->width == 0) && (node->height == 0)) 
				node->xloc = dxralign(node->xloc + node->width / 2) - node->width / 2;
			else 
				node->xloc = xralign(node->xloc + node->width / 2) - node->width / 2;
			if (node->xloc < oldx) 
				changed = 1;
		} 
		lnode = node;
		...
		if (diff > 0) {
			...
			if (rnode) {
				if ((node->width == 0) || (rnode->width == 0)) {
					if (node->xloc + node->width + G_dspace > rnode->xloc) 
						node->xloc = rnode->xloc - node->width - G_dspace;
				}
				else {
					if (node->xloc + node->width + G_xspace > rnode->xloc) 
						node->xloc = rnode->xloc - node->width - G_xspace;
				}
			} if ((node->width == 0) && (node->height == 0)) 
				node->xloc = dxlalign(node->xloc + node->width / 2) - node->width / 2;
			else 
				node->xloc = xlalign(node->xloc + node->width / 2) - node->width / 2;
			if (node->xloc > oldx) 
				changed = 1;
		} 
		rnode = node;
...
static int gs_ide1012(GEDGE edge)
{
	...
	start = edge->start;
	ende = edge->end;
	x1 = ende->xloc + (ende->width * edge->weightp / (ende->weightp + 1));
	x2 = start->xloc + (start->width * edge->weights / (start->weights + 1));
	return(x1 - x2);
}
static int gs_ide1011(GEDGE edge)
{
...
static int gs_ide1035(GNODE node)
{
	...
	nr_edges = 0;
	while (a) {
		h = (a->kante->priority * layout_downfactor) * gs_ide1012(a->kante);
		weight += h;
	...
	if (c) {
		if (c->target) {
			if (c->target->xloc < (node->xloc)) 
				weight -= (c->edge->priority * layout_nearfactor * (node->xloc - (c->target->xloc + (c->target->width) + G_xspace)));
			else 
				weight += (c->edge->priority * layout_nearfactor * (c->target->xloc - (node->xloc + (node->width) + G_xspace)));
			nr_edges += (c->edge->priority * layout_nearfactor);
		} if (c->target2) {
			if (c->target2->xloc < (node->xloc)) 
				weight -= (c->edge2->priority * layout_nearfactor * (node->xloc - (c->target2->xloc + (c->target2->width) + G_xspace)));
			else 
				weight += (c->edge2->priority * layout_nearfactor * (c->target2->xloc - (node->xloc + (node->width) + G_xspace)));
...
static int gs_ide1033(GNODE node)
{
	...
	nr_edges = 0;
	while (a) {
		h = (a->kante->priority * layout_upfactor) * gs_ide1011(a->kante);
		weight += h;
...
static int gs_ide1030(GNODE node)
{
	...
	weight = 0;
	nr_edges = 0;
	if ((layout_downfactor == 1) && (layout_upfactor == 1) && (node->succ) && (node->succ->next == 0) && (node->pred) && (node->pred->next == 0) && (node->succ->kante->priority == node->pred->kante->priority) && (node->connection) == NULL) {
		edge = (node->pred->kante);
		v = (edge->start);
		p1 = (node->xloc) + (node->width * (edge->weightp) / (node->weightp + 1));
		p2 = (v->xloc) + (v->width * (edge->weights) / (v->weights + 1));
		dx1 = p1 - p2;
		edge = (node->succ->kante);
		v = (edge->end);
		p1 = (node->xloc) + (node->width * (edge->weights) / (node->weights + 1));
		p2 = (v->xloc) + (v->width * (edge->weightp) / (v->weightp + 1));
		dx2 = p2 - p1;
		p1 = (node->yloc) + (node->height) / 2;
		p2 = (node->pred->kante->start->yloc) + (node->pred->kante->start->height) / 2;
		dy1 = p1 - p2;
		p2 = (node->succ->kante->end->yloc) + (node->succ->kante->end->height) / 2;
		dy2 = p2 - p1;
		weight = (dx2 * dy1 - dx1 * dy2) / (dy1 + dy2);
		return(weight);
	} a = (node->succ);
	while (a) {
		weight += (gs_ide1012(a->kante) * (a->kante->priority) * layout_downfactor);
		nr_edges += (a->kante->priority * layout_downfactor);
		a = (a->next);
	}   a = (node->pred);
	while (a) {
		weight += (gs_ide1011(a->kante) * (a->kante->priority) * layout_upfactor);
		nr_edges += (a->kante->priority * layout_upfactor);
		a = (a->next);
	} c = (node->connection);
	if (c) {
...
static int gs_ide1031(GNODE node, GNODE lnode, GNODE rnode)
{
	...
	if (node->pred) pred = node->pred->kante->start;
	else return(act_nw);
	if (node->succ) succ = node->succ->kante->end;
	else return(act_nw);
	ax = (pred->xloc);
	ay = (pred->yloc);
	mx = (node->xloc);
